...
export function DiffView({ diff }: { diff: DiffResult | null }) {
  const rendered = diff
    ? diff.hunks.map((h, hi) => {
        ...
        return (
          <div key={`hunk-${hi}`} className="diff-wrap" style={{ marginTop: 12 }}>
            <div className="diff-header">
              hunk {hi + 1} — A:{h.a_start},{h.a_lines} B:{h.b_start},{h.b_lines}
            </div>
            <div className="diff-body two-pane">{rows}</div>
          </div>
        );
      })
    : null;

  return <>{rendered ?? <em>No changes (or binary/missing)</em>}</>;
}
